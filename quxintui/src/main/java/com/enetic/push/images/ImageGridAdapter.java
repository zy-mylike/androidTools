...
public class ImageGridAdapter extends BaseAdapter {
	...
	int chosed=0;
	public void setChosed(int count){
		this.chosed=count;
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			holder = new Holder();
			convertView = View.inflate(act, R.layout.layout_item_image_grid, null);
		...
		if (item.isSelected) {
			...
			holder.selected.setImageResource(R.mipmap.icon_box_);
		...
		holder.iv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (( Bimp.getBm().drr.size() + selectTotal) < 9-chosed) {
				...
				} else if (( Bimp.getBm().drr.size() + selectTotal) >= 9-chosed) {
