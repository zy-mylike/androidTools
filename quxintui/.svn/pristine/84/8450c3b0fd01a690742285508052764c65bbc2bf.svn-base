...
public class PhotoActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		for (int i = 0; i < Bimp.getBm().bmp.size(); i++) {
			bmp.add( Bimp.getBm().bmp.get(i));
		}
		for (int i = 0; i <  Bimp.getBm().drr.size(); i++) {
			drr.add( Bimp.getBm().drr.get(i));
		}
		max =  Bimp.getBm().max;
		...
		photo_bt_del.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (listViews.size() == 1) {
					Bimp.getBm().bmp.clear();
					Bimp.getBm().drr.clear();
					Bimp.getBm().max = 0;
					FileUtils.deleteDir();
					finish();
				} else {
